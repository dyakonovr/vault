...
### In-memory блокировки (Mutex)

**Проблема:** `SELECT ... FOR UPDATE` гарантирует согласованность, но блокирует строки в базе данных и создаёт нагрузку на PostgreSQL при высоком конкурентном доступе к одному кошельку. Для одного инстанса приложения можно использовать более дешёвую локальную блокировку в памяти.

**Решение — `WalletLockManager`:**  
Менеджер хранит по одному `sync.Mutex` на каждый `walletID` в map. Он позволяет сериализовать операции с конкретным кошельком внутри процесса, не обращаясь к БД за блокировкой.

- **Структура:** `map[int64]*walletLockEntry`, где `walletLockEntry` содержит сам мьютекс и счётчик ссылок `refs` (сколько горутин сейчас ожидают или держат этот мьютекс).
- **Захват:** при вызове `Lock(ctx, walletID, timeout)`:
  - Под внешним мьютексом (`mu`) находится или создаётся запись для `walletID`, увеличивается `refs`.
  - Затем в цикле с таймаутом вызывается `TryLock()`. Если мьютекс занят, горутина спит 10 мс и повторяет, пока не истечёт `timeout`.
  - Проверяется `ctx.Err()`, чтобы корректно выйти при отмене запроса.
  - При успехе возвращается функция `release`, обёрнутая в `sync.Once` для защиты от повторного вызова.
- **Освобождение:** `release` сначала вызывает `Unlock()`, затем под внешним мьютексом уменьшает `refs`. Если `refs == 0`, запись удаляется из map.

**Ограничения:**  
- Работает **только в рамках одного инстанса**. При горизонтальном масштабировании (несколько реплик) локальный мьютекс не синхронизирует их между собой.
- Для кластера потребуется распределённая блокировка (Redis Redlock) или сохранение `FOR UPDATE` в БД. Возможна комбинация: локальный мьютекс для снижения contention внутри инстанса + распределённая блокировка для глобальной согласованности.

**Как встроено в use case (на примере `MutexDepositUsecase`):**
- Проверка владения кошельком (`IsOwnedBy`) до блокировки.
- Захват `walletLocker.Lock(ctx, walletID, timeout)` **до** начала транзакции БД.
- Вызов `defer unlock()` гарантирует освобождение после завершения операции.
- Проверка идемпотентности (`GetByIdempotencyKey`) выполняется **внутри транзакции и после захвата мьютекса**, чтобы предотвратить дублирование при параллельных запросах с одинаковым ключом.
- Блокировка удерживается на протяжении всей транзакции, включая коммит, что исключает гонки с другими горутинами.

**Ключевые решения и компромиссы:**
- **Захват до транзакции:** чтобы мьютекс покрывал коммит, иначе другая горутина могла бы начать операцию до фиксации первой.
- **Проверка идемпотентности после блокировки:** иначе два параллельных запроса с одинаковым ключом оба пройдут проверку до блокировки и создадут дубликат.
- **Порядок `Unlock` перед `releaseRef`:** чтобы до удаления записи из map мьютекс был уже разблокирован, и новая горутина не могла получить «висящий» замок.
- **`sync.Once` в release:** защита от паники при повторном освобождении.
- **Очистка map через `refs`:** запись удаляется только когда никто не ждёт и не держит мьютекс, чтобы map не разрастался бесконечно.
- **`TryLock` с таймаутом вместо блокирующего `Lock`:** позволяет возвращать ошибку `ErrLockTimeout`, если кошелёк занят слишком долго, сохраняя отзывчивость API.
